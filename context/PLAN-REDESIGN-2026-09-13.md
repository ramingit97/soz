...
**Вторая волна уточнений (2026-09-14, после просмотра веб-превью владельцем):**

8. **Имя персонажа по умолчанию — Bobo / Бобо** (не Хани). Меняется везде: UI,
   подстановка в `utils/companion.ts`, тексты сторов и `app.config.ts`. Имя,
   которое дал ребёнок, по-прежнему главнее. Бренд приложения — Söz.
9. **Персонаж зависит от возраста:** режим `kid` (5–10) — медвежонок, режим
   `teen` (11+ и взрослые) — робот. Имя одно. B1 рисует два персонажа; серверный
   промпт (`system-prompts.ts`, сейчас «медвежонок» для всех детей) переводится
   на тот же порог возраста. Отнесение подростков к «роботу» — моя трактовка слов
   владельца «если ребёнок — медведь, иначе робот», согласована с режимами.
10. **Регистрация и код родителя не в онбординге.** Гостевой аккаунт создаётся сам
    в конце, регистрация — со 2-го дня; PIN родителя спрашивается при первом входе
    в родительский раздел.
11. **Упор плана выбирается в начале:** «Говорить / Слушать / Всё понемногу»
    (одиночный выбор, по умолчанию «всё понемногу»). «Слушать» требует правки
    генератора: `lessonPrefs` сейчас знает только `moreTalk` и `moreWords`.
12. **Главный экран предельно простой.** Одна карточка «Урок дня», внутри неё шаги
    плана на сегодня (слова → история → разговор), тап продолжает с текущего шага.
    Отдельная страница «План дня» уходит. Плитки «Поговорить на тему» и «Послушать
    историю» не отдельные — это шаги плана, их доля зависит от упора. «Серия»,
    «Слова», «Что помнит персонаж» — во вкладку «Прогресс». Раздел E ниже в части
    плиток и сетки «Ещё» заменяется этим пунктом.
13. **Вкладка «Говорить» остаётся** — свободный разговор в любой момент.
14. **Молчаливые отказы — баг.** Экран, которому нужен аккаунт (история на слух и
    т. п.), не должен молча ничего не делать: показывать, что происходит.
...
| Батч | Состояние | Подробности |
|---|---|---|
| B0 | сделан 2026-09-14, коммит `762465c`; опубликован в `preview` вместе с B2/B3 | `SESSION-2026-09-14-handoff.md` |
| B2 + имя Bobo + сервер | сделано 2026-09-14, коммит `53dfa1a` (в `main`); API на Fly; опубликовано в `preview` | `SESSION-2026-09-14-handoff.md` |
| B3 главный (по пункту 12) | сделано 2026-09-14, там же; опубликовано в `preview` | `SESSION-2026-09-14-handoff.md` |
| B1 маскот | сделан 2026-09-15, коммит `8bd0fa9`; опубликован в `preview`; ждёт трёх раундов владельца на телефоне. Тело целиком от 100 dp (не 64) | `SESSION-2026-09-15-handoff.md` |
| B4 разговор | сделан 2026-09-15, там же; опубликован в `preview` | `SESSION-2026-09-15-handoff.md` |
| B5 уроки | сделан 2026-09-16, коммит `335a9d7`; опубликован в `preview` (`70435a03…`). Общая шапка шага вместо `LessonProgress`; тексты уроков переведены на язык интерфейса; встроенные уроки для малышей — про медвежонка | `SESSION-2026-09-15-handoff.md` |
| B6 родитель и пейволл | сделан 2026-09-18, коммит `081bbc3`; опубликован в `preview` (`6686893e…`). `parent-settings` удалён (ни одной ссылки, фальшивые переключатели); расписание и цели открываются из родительского раздела с `?from=parent`; согласие — одна галочка + PIN; выдуманные отзывы на пейволле убраны | `SESSION-2026-09-15-handoff.md` |
| B7 хвост | сделан 2026-09-18, коммиты `92f74d6`, `5ca3c73`, `0ec076e`; опубликован в `preview` (`dd4c09c4…`). `Bobo.tsx`, `missed`, `language`, `parent-add-child`, `setup/pet`, `setup/notify` удалены; «Сохранить» в расписании и целях пишет на сервер; недостижимые экраны разобраны в `0e59875`: `setup/level`, `placement`, `topics` вернулись, `story`, `songs`, `photo-learn`, `setup/goal`, `interests` удалены | `SESSION-2026-09-15-handoff.md` |
...
**Состояние плана:** B0–B7 сделаны и опубликованы (таблица прогресса в разделе F
выше). Недостижимые экраны разобраны (`0e59875`). Точка входа для новой сессии —
