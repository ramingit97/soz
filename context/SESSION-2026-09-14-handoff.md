...
**Итог сессии:** всё закоммичено в `53dfa1a` (ветка `redesign-onboarding-home`,
влита в `main` перемоткой; пушит владелец) и опубликовано через EAS Update: канал `preview`,
Android, группа `9ac8ed9f-679f-4eed-9177-a68d538398ae`, runtime `1a02f268…`,
бандл с `soz-api.fly.dev`. На телефоне со сборкой `f3eb38b0` обновление
применяется со второго запуска приложения.

**Вечер того же дня:** запись и голос Бобо в браузере (`8f15127`), затем батч
«голос нажатием, продолжение урока, быстрее ответ» (`5802448`) — выложен
полностью: API на Fly, EAS Update `preview` группа
`b7f9e000-390a-47a1-91b1-8ea003e7cf63`. Подробно — раздел «Вечер 2026-09-14» ниже.
Пушит владелец сам.

Открытые вопросы владельцу (ждём его проверки на телефоне):
- квест: видна ли кнопка микрофона целиком после правки раскладки;
- «Говорить»: ответ стал быстрее, запись останавливается сама;
- урок: после «Yeni sözlər» повторный вход открывает следующий шаг,
  на карточке «Продолжить урок»;
- «три фразы в уроке-разговоре → урок засчитан».

Первое действие следующей сессии: `git status` (в `babel.config.js` не должно
быть правки `unstable_transformImportMeta` — см. ловушку 7), затем спросить
владельца, что показала проверка на телефоне.
...
## Вечер 2026-09-14: голос в браузере, запись нажатием, продолжение урока

### `8f15127` — запись и голос Бобо в веб-превью

Владелец: «голос как будто меня не слышит». Экраны читали запись через
`expo-file-system`, которого в браузере нет; ошибка проглатывалась. Новый
`utils/recording.ts`: `readAsBase64(uri)` (в браузере — `fetch(blob)` +
`FileReader`, отдаёт настоящий mime) и `playableAudioUri(base64, mime)` (в браузере —
data-URI, на телефоне — временный файл). `postWordCheck` принимает mime. Квест
показывает ошибку отправки вместо тишины. `+html.tsx`: высота `100dvh`.

### «План не собирается» — кончились деньги OpenAI

`setup/building` писал «Не получилось» — баланс OpenAI ушёл в минус (−$0.21),
владелец пополнил на $6. Сервер глотает ошибку генерации плана (`.catch(() => {})`),
а экран советует «проверь интернет». **Предложено, ждёт согласия:** логировать эти
ошибки на сервере и писать на экране «Сервис временно недоступен».

### `5802448` — пять жалоб владельца

| Жалоба | Причина | Исправление |
|---|---|---|
| Бобо долго отвечает | Safari пишет mp4, OpenAI отвечал «Audio file might be corrupted», сервер распознавал заново через Deepgram — 6–9 с. `/talk` делал всё по очереди: проверка безопасности (LLM), память, интересы, история, ответ, запись, повторное чтение истории, голос | Браузер шлёт WAV 16 кГц моно (`hooks/useVoiceRecorder.web.ts`, `utils/wav.ts`). `/talk`: проверка безопасности, чтения из БД и ответ Бобо стартуют вместе; реплика ребёнка пишется во время генерации, ответ — во время синтеза голоса. Ответ используется **только после** проверки безопасности, при кризисе выбрасывается |
| На микрофон трудно нажать | Нужно было держать; на «Говорить» кнопку наполовину закрывали вкладки | Нажатие (`MicButton onPress`); `BottomTabsSpacer` под микрофоном на `talk` |
| Не понимает, что я замолчал | Определения конца речи не было | `utils/speechEnd.ts`: 300 мс калибровка шума, порог = шум + 10 дБ (в пределах −55…−30), речь — 2 громких замера подряд, конец — тишина 1,2 с (квест 0,8 с), без речи — 7 с, максимум 15 с. Телефон — метр `expo-audio` (`hooks/useVoiceRecorder.ts`), браузер — Web Audio. Экраны `talk`, `lesson/quest`, `lesson/world`, `lesson/pretend` |
| UI уходит под экран | «Говорить» — вкладки поверх микрофона; квест на низком экране (664 px) — кнопка за нижним краем; жёлтая виньетка резала пузыри жёстким краем на 180 dp | Середина квеста, «Покажи мир», ролевой игры — `ScrollView`, микрофон и подсказка всегда видны; при высоте окна < 760 Бобо 72 вместо 110; обёртка `MicButton` 160 вместо 200; виньетка — градиент |
| После «Yeni sözlər» урок снова сначала | Пройденные шаги урока не сохранялись, главный всегда открывал первый | `lessonStepsDone` в сторе (ключ `childId:lang:day`, старые дни чистятся), `markLessonStep` в конце `word-game` (words), `grammar`, `read` (reading, только mature); `resumeStep` + `lessonStepRoute` на главном; `HomeHero` — галочки шагов и «Продолжить урок» |

Попутно:
- **`Alert.alert` в react-native-web — пустая функция**: все ошибки и
  подтверждения в веб-превью молчали. `utils/webAlert.ts` (`installWebAlert` в
  `_layout`) — `window.alert` / `window.confirm`.
- `utils/talkAlert.ts` — общий «Бобо не ответил. Проверь интернет…» / «Бобо устал»
  (лимит) для `talk`, `world`, `pretend`; раньше в `world`/`pretend` сбой молчал, в
  `talk` был английский «Make sure the API is running».
- Тень `MicButton` скруглена (в браузере был квадрат), фишка задержки в `talk`
  только в `__DEV__`, оценка длины аудио на сервере учитывает WAV (32 КБ/с).

Проверки: типы (mobile, api), тесты mobile 51 и api 59, `check-ui` ОК,
`expo export --platform android`, отпечаток Android `1a02f268…` не изменился. В
headless Chrome с записанной фразой (0,8 с тишины + 5,4 с речи + 4 с тишины):
`talk`, квест, «Покажи мир», ролевая игра — после одного нажатия запись сама
остановилась через паузу и ушла как `audio/wav`; хвост тишины в WAV ровно 1,2 с;
OpenAI распознал отправленный WAV за 1,5 с слово в слово; диалог ошибки
показывается.

**Не проверено:** квест после правки раскладки (headless Chrome перестал
открывать страницы — ловушка 14); продолжение урока кликами (нужен план с
сервера, покрыто тестами `resumeStep` и `markLessonStep`); автоостановка на
Android на устройстве (если метр не отдаёт громкость — остановка через 15 с или
нажатием); выигрыш `/talk` по времени на живом разговоре не замерен.

### Ловушки вечера

10. **`Alert.alert` на вебе ничего не делает** — см. выше, закрыто `installWebAlert`.
11. **Safari `MediaRecorder` → `audio/mp4`, OpenAI его не принимает.** Веб теперь
    шлёт WAV. На сервере `transcribeOpenAI` по-прежнему считает всё, что не
    mp3/wav, файлом `m4a` (webm из Chrome — тоже), сейчас веб это не задевает.
12. **Платформенные файлы Metro:** `useVoiceRecorder.web.ts` рядом с
    `useVoiceRecorder.ts`, типы берутся из `.ts`.
13. **Фиктивный микрофон в headless Chrome:** `--use-fake-device-for-media-stream
    --use-fake-ui-for-media-stream --use-file-for-fake-audio-capture=phrase.wav%noloop`.
    WAV от OpenAI TTS идёт с размерами `0xFFFFFFFF` в заголовке — переписать
    заголовок. Нажатие в RN Web — `element.click()` на ближайшем `[tabindex]`
    (события мыши/тача через CDP не срабатывают). JS-диалоги закрывать
    `Page.handleJavaScriptDialog`, иначе страница висит.
14. **`chrome.kill()` убивает только обёртку `google-chrome`.** К концу сессии
    headless Chrome перестал загружать любые http-страницы (запрос уходит, ответа
    нет; `data:` открывается) — с новым профилем, без песочницы, с разными
    флагами. Причину не нашёл. Не убивать процессы по шаблону `chrome`: на машине
    открыт обычный Chrome владельца.
15. **EAS Update может погибнуть при перезапуске сессии** (exit 137) — запускать в
    фоне и проверять `Published!` в логе.

### На потом

- Ждёт согласия: логирование проглоченных ошибок генерации; честный текст на
  `setup/building`.
- `AdultHome`; `lesson/complete` по-английски для `lang=en` (B5); встроенные уроки
  «я добрый робот» у малышей (B5); маскот (B1); текст запроса микрофона с «Хани» в
  `app.config.ts` (нативный, до следующей сборки); лимит `/auth/guest` до релиза.
- Скрипты превью (`serve-preview.mjs`, `walk.mjs`, `tap-test.mjs`) жили в scratchpad
  сессии и пропадут; Cloudflare-ссылка тоже.
...
- `talk`: своя кнопка «‹» вместо `HBBackButton`. B4. (Микрофон под вкладками —
  исправлено в `5802448`.)
