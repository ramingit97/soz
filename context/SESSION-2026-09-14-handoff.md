...
**Итог сессии:** всё закоммичено в `53dfa1a` на ветке `redesign-onboarding-home`
(не запушена, в `main` не влита) и опубликовано через EAS Update: канал `preview`,
...
Открытые вопросы владельцу: запушить ветку и влить в `main`; проверить на
телефоне сценарий «три фразы в уроке-разговоре → урок засчитан» (в веб-превью нет
микрофона).
